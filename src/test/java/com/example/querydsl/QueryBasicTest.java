...
//                .fetchOne();
        ...
        results.getTotal(); // total Count를 가져와야 하기 때문에 쿼리가 2번 실행됩니다.
        results.getResults(); // 이렇게 getResults()를 해주어야 결과가 나옵니다.
    ...
    public void aggregation() {
        List<Tuple> result = queryFactory // 실무에선 tuple을 많이 사용하진 않습니다.
                .select(
                        member.count(),
                        member.age.sum(),
                        member.age.avg(),
    ...
    void join_on_filtering() {
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(member.team, team) // outer join
    ...
    @Test
    void join_on_no_relation() { // 세타 조인은 외래키가 없어도 사용이 가능하다.
        em.persist(new Member("teamA"));
        em.persist(new Member("teamB"));
    ...
    public void constant() {
        List<Tuple> result = queryFactory
                .select(member.username, Expressions.constant("A")) // 상수가 튀어나오게 된다.
                .from(member)
                .fetch();
    ...
    public void concat() {
        ...
        // stringValue() 문자가 아닌 다른 타입들은 stringValue()로 문자를 변환가능
    ...
    @Test
    public void tuple() { // DTO로 변환시켜서 나가야 한다.
        List<Tuple> result = queryFactory
    ...
    @Test
    public void findDtoBySetter() { // Projections.bean은 기봔 생성자를 사용하여 DTO 객체를 생성한 후, setter를 이용하여 필드 값을 주입합ㅂ니ㅏㄷ.
    ...
    public void dynamicQuery_BooleanBuilder() {
        ...
        List<Member> result2 = searchMember1(null, null);
    ...
    /**
     * Where - 다중 파라미터 (실무 주로 사용)
     * where문 내부에서 바로 해결을 한다.
     * where절에 null이 오는 경우, 그냥 무시 됩니다.
     * 이 방식이 제일 깔끔합니다.
     * null이라 하는 경우 where에서 무시를 해버립니다.
     * 이렇게 하는 경우 하나의 메서드로 조립이 가능합니다.
     */
    @Test
